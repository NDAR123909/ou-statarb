...
class BacktestConfig:
    ...
    stop_z: float | None = None  # structural-break stop: force-exit if the spread
    #                              keeps diverging past this (e.g. 3.5-4.0). After a
    #                              stop, re-entry on the SAME side is blocked until
    #                              z heals back inside the entry band. This is the
    #                              escape hatch for the LTCM failure mode.
    legs_cost_mult: float = 1.0  # cost multiplier for the full two-leg trade.
    #                              Set to (1 + |beta|) so every unit of spread
    #                              turnover pays for BOTH legs. Default 1.0 keeps
    #                              old single-leg behaviour for comparability.
...
def backtest(
    ...
    pos = np.zeros(n)
    hold = 0
    blocked = 0   # +1: longs blocked after a stop, -1: shorts blocked, 0: free
    ...
    for t in range(1, n):
        ...
        # a stopped side unblocks once the spread heals back inside the band
        if blocked == +1 and z[t] > -cfg.entry_z:
            blocked = 0
        elif blocked == -1 and z[t] < cfg.entry_z:
            blocked = 0

        if prev == 0:
            # flat -> look for entry. Spread rich (z high) => short (-1);
            # spread cheap (z low) => long (+1).
            if z[t] > cfg.entry_z and blocked != -1:
                pos[t] = -1.0
                hold = 0
            elif z[t] < -cfg.entry_z and blocked != +1:
                pos[t] = +1.0
                hold = 0
            else:
                pos[t] = 0.0
            ...
            stopped = cfg.stop_z is not None and (
                (prev > 0 and z[t] < -cfg.stop_z) or
                (prev < 0 and z[t] > cfg.stop_z))
            if stopped:
                pos[t] = 0.0
                blocked = +1 if prev > 0 else -1
    ...
    cost = turnover * cfg.legs_cost_mult * (cfg.cost_bps / 1e4)
